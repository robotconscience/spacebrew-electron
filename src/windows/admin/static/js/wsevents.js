...
var handleMessageMsg = function(msg){
	...
	if (fromEndpoint){
		var getImage = function(active){
			return "static/img/node-"+($("#"+itemSelector).attr('class').indexOf('sub_') < 0 
				? "open" 
...
var addConnection = function(msg){
	...
	source.setImage("static/img/node-closed.png");
	target.setImage("static/img/node-closed.png");
...
var handleUnselecting = function(pubId, subId){
	...
	if (subscriber.attr('class').indexOf('pub_') < 0){
		myPlumb.endpoints[subId].setImage("static/img/node-open.png");
	}
	if (publisher.attr('class').indexOf('sub_') < 0){
		myPlumb.endpoints[pubId].setImage("static/img/node-open.png");
	}
};
